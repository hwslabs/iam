  ...
  class CreateOrganizationRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IamRuby::CreateOrganizationRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IamRuby::CreateOrganizationRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @admin_user.nil?
        invalid_properties.push('invalid value for "admin_user", admin_user cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @name !~ Regexp.new(/^[A-Za-z0-9]{3,50}$/)
    ...
    def hash
      [name, description, admin_user].hash
    end
