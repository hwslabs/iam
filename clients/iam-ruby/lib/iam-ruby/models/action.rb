  ...
  class Action
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IamRuby::Action` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IamRuby::Action`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @resource_name.to_s.length > 50
        invalid_properties.push('invalid value for "resource_name", the character length must be smaller than or equal to 50.')
      end
    ...
    def valid?
      ...
      return false if @resource_name.to_s.length > 50
    ...
    def resource_name=(resource_name)
      ...
      if resource_name.to_s.length > 50
        fail ArgumentError, 'invalid value for "resource_name", the character length must be smaller than or equal to 50.'
      end
    ...
    def hash
      [organization_id, resource_name, name, hrn, description].hash
    end
